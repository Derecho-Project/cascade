...
derecho::rpc::QueryResults<version_tuple> ServiceClient<CascadeTypes...>::put_objects(
    ...
    if (!is_external_client()) {
        ...
        if (static_cast<uint32_t>(group_ptr->template get_my_shard<SubgroupType>(subgroup_index)) == shard_index) {
            // ordered put as a shard member
            auto& subgroup_handle = group_ptr->template get_subgroup<SubgroupType>(subgroup_index);
            return subgroup_handle.template ordered_send<RPC_NAME(ordered_put_objects)>(values,as_trigger);
        } else {
            // p2p put
            ...
            try {
                // as a subgroup member
                auto& subgroup_handle = group_ptr->template get_subgroup<SubgroupType>(subgroup_index);
                return subgroup_handle.template p2p_send<RPC_NAME(put_objects)>(node_id,values,as_trigger);
            } catch (derecho::invalid_subgroup_exception& ex) {
                // as an external caller
                auto& subgroup_handle = group_ptr->template get_nonmember_subgroup<SubgroupType>(subgroup_index);
                return subgroup_handle.template p2p_send<RPC_NAME(put_objects)>(node_id,values,as_trigger);
        ...
        auto& caller = external_group_ptr->template get_subgroup_caller<SubgroupType>(subgroup_index);
        node_id_t node_id = pick_member_by_policy<SubgroupType>(subgroup_index,shard_index,values[0].get_key_ref());
        return caller.template p2p_send<RPC_NAME(put_objects)>(node_id,values,as_trigger);
    }
}
    ...
    if (!is_external_client()) {
        ...
        if (static_cast<uint32_t>(group_ptr->template get_my_shard<SubgroupType>(subgroup_index)) == shard_index) {
            ...
            try {
                // as a subgroup member
                auto& subgroup_handle = group_ptr->template get_subgroup<SubgroupType>(subgroup_index);
                subgroup_handle.template p2p_send<RPC_NAME(put_objects_and_forget)>(node_id,values,as_trigger);
            } catch (derecho::invalid_subgroup_exception& ex) {
                // as an external caller
                auto& subgroup_handle = group_ptr->template get_nonmember_subgroup<SubgroupType>(subgroup_index);
                subgroup_handle.template p2p_send<RPC_NAME(put_objects_and_forget)>(node_id,values,as_trigger);
        ...
        auto& caller = external_group_ptr->template get_subgroup_caller<SubgroupType>(subgroup_index);
        node_id_t node_id = pick_member_by_policy<SubgroupType>(subgroup_index,shard_index,values[0].get_key_ref());
        caller.template p2p_send<RPC_NAME(put_objects_and_forget)>(node_id,values,as_trigger);
    }
}
...
derecho::rpc::QueryResults<version_tuple> ServiceClient<CascadeTypes...>::type_recursive_put_objects(
        ...
        bool as_trigger) {
    if (type_index == 0) {
        return this->template put_objects<FirstType>(values,subgroup_index,shard_index,as_trigger);
    } else {
        return this->template type_recursive_put_objects<ObjectType, SecondType, RestTypes...>(type_index-1,values,subgroup_index,shard_index,as_trigger);
    }
}
...
derecho::rpc::QueryResults<version_tuple> ServiceClient<CascadeTypes...>::type_recursive_put_objects(
        ...
        bool as_trigger) {
    if (type_index == 0) {
        return this->template put_objects<LastType>(values,subgroup_index,shard_index,as_trigger);
    } else {
        throw derecho::derecho_exception(std::string(__PRETTY_FUNCTION__) + ": type index is out of boundary.");
    }
}
...
void ServiceClient<CascadeTypes...>::type_recursive_put_objects_and_forget(
        ...
        bool as_trigger) {
    if (type_index == 0) {
        this->template put_objects_and_forget<FirstType>(values,subgroup_index,shard_index,as_trigger);
    } else {
        this->template type_recursive_put_objects_and_forget<ObjectType, SecondType, RestTypes...>(type_index-1,values,subgroup_index,shard_index,as_trigger);
    }
}
...
void ServiceClient<CascadeTypes...>::type_recursive_put_objects_and_forget(
        ...
        bool as_trigger) {
    if (type_index == 0) {
        this->template put_objects_and_forget<LastType>(values,subgroup_index,shard_index,as_trigger);
    } else {
        throw derecho::derecho_exception(std::string(__PRETTY_FUNCTION__) + ": type index is out of boundary.");
    }
}
...
derecho::rpc::QueryResults<void> ServiceClient<CascadeTypes...>::trigger_put(
        const ObjectType& value) {
    // STEP 1 - get key
    static_assert(std::is_base_of_v<ICascadeObject<std::string,ObjectType>,ObjectType>, "trigger_put(value) only supports objects of type ICascadeObject<std::string,ObjectType>");
...
ServiceClient<CascadeTypes...>::get_signature_by_version(const persistent::version_t& version,
    ...
    if(!is_external_client()) {
        std::lock_guard<std::mutex> lck(this->group_ptr_mutex);
        // Now that pick_member_by_policy requires a "key" argument, should I change get_signature_by_version to also require a key?
        node_id_t target_node_id = pick_member_by_policy<SubgroupType>(subgroup_index, shard_index, 0);
        try {
...
template <typename... CascadeTypes>
template <typename FirstType, typename SecondType, typename... RestTypes>
derecho::rpc::QueryResults<std::tuple<std::vector<uint8_t>, persistent::version_t>> ServiceClient<CascadeTypes...>::type_recursive_get_signature_by_version(
        uint32_t type_index,
        const persistent::version_t& version,
        uint32_t subgroup_index,
        uint32_t shard_index) {
    if (type_index == 0) {
        //This must be a runtime check, not a static_assert, because type_index is a runtime value
        if constexpr(is_signature_store<FirstType>::value) {
            return this->template get_signature_by_version<FirstType>(version,subgroup_index,shard_index);
        } else {
        ...
        return this->template type_recursive_get_signature_by_version<SecondType,RestTypes...>(type_index-1,version,subgroup_index,shard_index);
    }
}
...
derecho::rpc::QueryResults<std::tuple<std::vector<uint8_t>, persistent::version_t>> ServiceClient<CascadeTypes...>::type_recursive_get_signature_by_version(
    ...
    if (type_index == 0) {
        if constexpr(is_signature_store<LastType>::value) {
            return this->template get_signature_by_version<LastType>(version,subgroup_index,shard_index);
        } else {
            throw derecho::derecho_exception("get_signature_by_version can only be called on objects stored in SignatureCascadeStore subgroups");
        }
    } else {
...
template <typename... CascadeTypes>
template <typename KeyType>
derecho::rpc::QueryResults<std::tuple<std::vector<uint8_t>, persistent::version_t>> ServiceClient<CascadeTypes...>::get_signature_by_version(
                const KeyType& key,
    const persistent::version_t& version) {

    const auto [subgroup_type_index,subgroup_index,shard_index] = this->template key_to_shard(key);

    return this->template type_recursive_get_signature_by_version<CascadeTypes...>(subgroup_type_index,version,subgroup_index,shard_index);
...
ServiceClient<CascadeTypes...>::request_signature_notification(const persistent::version_t& version, uint32_t subgroup_index, uint32_t shard_index) {
    if(!is_external_client()) {
        std::lock_guard<std::mutex> lck(this->group_ptr_mutex);
        // Now that pick_member_by_policy requires a "key" argument, do I need to change request_signature_notification to require a key?
        node_id_t target_node_id = pick_member_by_policy<SubgroupType>(subgroup_index, shard_index, 0);
        try {
            auto& subgroup_handle = group_ptr->template get_subgroup<SubgroupType>(subgroup_index);
            if(static_cast<uint32_t>(group_ptr->template get_my_shard<SubgroupType>(subgroup_index)) == shard_index) {

...
version_tuple VolatileCascadeStore<KT, VT, IK, IV>::ordered_put(const VT& value, bool as_trigger) {
    ...
    try {
        auto ret = internal_ordered_put(value, as_trigger);
        ...
        debug_leave_func_with_value("version=0x{:x},timestamp={}us,previous_version=0x{:x},pervious_version_by_key=0x{:x}",
                                    std::get<0>(ret), std::get<1>(ret), std::get<2>(ret), std::get<3>(ret));
        return ret;
...
template <typename KT, typename VT, KT* IK, VT* IV>
version_tuple VolatileCascadeStore<KT, VT, IK, IV>::internal_ordered_put(const VT& value, bool as_trigger) {
    /* TODO to avoid code repetition, this could use the new vector version:
     * return this->internal_ordered_put_objects({value});
     */
...
#error Lockless support is currently for GCC only
    ...
    return {this->update_version, std::get<1>(version_and_hlc).m_rtc_us, previous_version, previous_version_by_key};
}

...
public:
  ...
  ObjectPoolPathINode (fuse_ino_t pino, ServiceClientAPI& _capi):
    capi(_capi){
...
public:
    ObjectPoolRootINode (ServiceClientAPI& _capi, fuse_ino_t pino=FUSE_ROOT_ID) :
      ObjectPoolPathINode(pino, _capi) {
...
public:
    MetadataServiceRootINode(ServiceClientAPI& _capi) :
        capi(_capi) {
...
public:
    DataPathLogicRootINode (ServiceClientAPI& _capi, fuse_ino_t pino) :
        capi(_capi) {
...
class FuseClientContext {
    template<typename CascadeType> using _CascadeTypeINode = CascadeTypeINode<CascadeType>;
private:
    /** initialization flag */

...
from util import (wait_for_mount, compare_dirs, umount, cleanup, base_cmdline,
                  safe_sleep, basename,  test_printcap,
                  fuse_proto, powerset)
...
objp_expected_third_level = [[], [".cascade", "a1"], [".cascade", "b1"], [".cascade", "c1"]]
objp_expected_keys = [[], [".cascade", "key0"], [".cascade", "key1"], [".cascade", "c2"]]
...
def initial_setup():
    ...
    capi.put(put_keys[0], bytes(put_values[0],'utf-8'), subgroup_type='VolatileCascadeStoreWithStringKey', subgroup_index=0, shard_index=0)
...
def tst_sg_dirs(mnt_dir, run_id=""):
    ...
    expected_first_level =  ['.cascade', 'MetadataService', 'PersistentCascadeStoreWithStringKey', 'VolatileCascadeStoreWithStringKey', 'TriggerCascadeNoStoreWithStringKey','ObjectPools' ]
    ...
    for subgroup_type in expected_first_level:
        if(subgroup_type=='.cascade' or subgroup_type=='MetadataService' or subgroup_type=='ObjectPools'):
            continue
        ...
        mnt_third_level = os.listdir(mnt_dir + "/" + subgroup_type + "/" + sg_expected_second_level[0])
        ...
        if(subgroup_type == 'PersistentCascadeStoreWithStringKey'):
            mnt_forth_level = os.listdir(mnt_dir + "/" + subgroup_type + "/" + sg_expected_second_level[0] + "/" + sg_expected_third_level[0])
            error_m = error_msg(run_id, "PCSS forthlevel", mnt_forth_level, pcss_expected_keys)
...
def tst_objp_dirs(mnt_dir, run_id=""):
    ...
    error_m = error_msg(run_id, "object pool test", mnt_objp_root, objp_expected_second_level)
    assert compare_dirs(mnt_objp_root, objp_expected_second_level), error_m
    for i in range(1,4):
        mnt_objp_dir = os.listdir(mnt_dir + "/" + "ObjectPools" + "/" + objp_expected_second_level[i])
        error_m = error_msg(run_id, f"object pool test{str(i)}", mnt_objp_dir, objp_expected_third_level[i])
        assert compare_dirs(mnt_objp_dir, objp_expected_third_level[i])
        mnt_objp_key = os.listdir(mnt_dir + "/" + "ObjectPools" + "/" + objp_expected_second_level[i] + "/" + objp_expected_third_level[i][1])
        assert compare_dirs(mnt_objp_key, objp_expected_keys[i])
        if(i == 3):
            mnt_objp_subdir = os.listdir(mnt_dir + "/" + "ObjectPools" + "/" + objp_expected_second_level[i] + "/" + objp_expected_third_level[i][1] + "/" + objp_expected_keys[i][1])
...
def tst_content(mnt_dir, run_id=""):
    ...
    for i in range(1,2):
        sg_mnt_key = mnt_dir + subgroup_type + "subgroup-0/shard-0/" +vcss_expected_keys[i+1]
        with open(sg_mnt_key, 'r') as fh:
            output = fh.read()
            error_m = error_msg(run_id, f"sg content{sg_mnt_key}", output, put_values[i])
            assert output == put_values[i], error_m
    subgroup_type = "/PersistentCascadeStoreWithStringKey/"
    for i in range(2,4):
        sg_mnt_key = mnt_dir + subgroup_type + "subgroup-0/shard-0/" +pcss_expected_keys[i-1]
        with open(sg_mnt_key, 'r') as fh:
            output = fh.read()
            error_m = error_msg(run_id, f"sg content{sg_mnt_key}", output, put_values[i])
            assert output == put_values[i], error_m
    ...
    for i in range(1,4):
        ...
        with open(objp_mnt_key, 'r') as fh:
            output = fh.read()
            error_m = error_msg(run_id, f"objp content{objp_mnt_key}", output, put_values[i])
            assert output == put_values[i], error_m
...
def cascade_fuse_mount(mnt_dir):
    ...
    mount_process = subprocess.Popen(cmd_input, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    # try:
...
def main(argv):
    ...
    mnt_dir = pjoin(basename,"testdir")  # use build/src/service/fuse/testdir as mounting point
    ...
    for i in range(process_num):
        ...
        proc = multiprocessing.Process(target=fuse_test_cases, args=(mnt_dir,run_id,))
    ...
    for i in range(thread_num):
        ...
        thread = threading.Thread(target=fuse_test_cases, args=(mnt_dir,run_id,))

...
#define PROC_NAME   "cascadechain_c"

template <typename SubgroupType>
void print_shard_member(ChainServiceClient& capi, uint32_t subgroup_index, uint32_t shard_index) {
...
template <typename SubgroupType>
void print_member_selection_policy(ChainServiceClient& capi, uint32_t subgroup_index, uint32_t shard_index) {
    std::cout << "Subgroup (Type=" << std::type_index(typeid(SubgroupType)).name() << ","
              << "subgroup_index=" << subgroup_index << ","
...
template <typename SubgroupType>
void set_member_selection_policy(ChainServiceClient& capi, uint32_t subgroup_index, uint32_t shard_index,
...
/* TEST1: members */
void member_test(ChainServiceClient& capi) {
    // print all members.
    ...
    print_shard_member<SignatureCascadeStoreWithStringKey>(capi,0,0);
}
...
template <typename SubgroupType>
void put(ChainServiceClient& capi, const std::string& key, const std::string& value, persistent::version_t pver, persistent::version_t pver_bk, uint32_t subgroup_index, uint32_t shard_index) {
...
template <typename SubgroupType>
void put_and_forget(ChainServiceClient& capi, const std::string& key, const std::string& value, persistent::version_t pver, persistent::version_t pver_bk, uint32_t subgroup_index, uint32_t shard_index) {
    typename SubgroupType::ObjectType obj;
    if constexpr (std::is_same<typename SubgroupType::KeyType,uint64_t>::value) {
        obj.key = static_cast<uint64_t>(std::stol(key));
    } else if constexpr (std::is_same<typename SubgroupType::KeyType,std::string>::value) {
...
void op_put(ChainServiceClient& capi, const std::string& key, const std::string& value, persistent::version_t pver, persistent::version_t pver_bk) {
...
void op_put_and_forget(ChainServiceClient& capi, const std::string& key, const std::string& value, persistent::version_t pver, persistent::version_t pver_bk) {
    ObjectWithStringKey obj;
    obj.key = key;
    obj.previous_version = pver;
    obj.previous_version_by_key = pver_bk;
...
template <typename SubgroupType>
void create_object_pool(ChainServiceClient& capi, const std::string& id, uint32_t subgroup_index) {
...
template <typename SubgroupType>
void trigger_put(ChainServiceClient& capi, const std::string& key, const std::string& value, uint32_t subgroup_index, uint32_t shard_index) {
    typename SubgroupType::ObjectType obj;
    if constexpr (std::is_same<typename SubgroupType::KeyType,uint64_t>::value) {
        obj.key = static_cast<uint64_t>(std::stol(key));
    } else if constexpr (std::is_same<typename SubgroupType::KeyType,std::string>::value) {
...
void op_trigger_put(ChainServiceClient& capi, const std::string& key, const std::string& value) {
    ObjectWithStringKey obj;
...
template <typename SubgroupType>
void collective_trigger_put(ChainServiceClient& capi, const std::string& key, const std::string& value, uint32_t subgroup_index, std::vector<node_id_t> nodes) {
    typename SubgroupType::ObjectType obj;
    if constexpr (std::is_same<typename SubgroupType::KeyType,uint64_t>::value) {
        obj.key = static_cast<uint64_t>(std::stol(key));
    } else if constexpr (std::is_same<typename SubgroupType::KeyType,std::string>::value) {
...
template <typename SubgroupType>
void remove(ChainServiceClient& capi, const std::string& key, uint32_t subgroup_index, uint32_t shard_index) {
    if constexpr (std::is_same<typename SubgroupType::KeyType,uint64_t>::value) {
...
void op_remove(ChainServiceClient& capi, const std::string& key) {
...
template <typename SubgroupType>
void get(ChainServiceClient& capi, const std::string& key, persistent::version_t ver, bool stable, uint32_t subgroup_index,uint32_t shard_index) {
    if constexpr (std::is_same<typename SubgroupType::KeyType,uint64_t>::value) {
...
template <typename SubgroupType>
void get_by_time(ChainServiceClient& capi, const std::string& key, uint64_t ts_us, uint32_t subgroup_index, uint32_t shard_index) {
    if constexpr (std::is_same<typename SubgroupType::KeyType,uint64_t>::value) {
...
template <typename SubgroupType>
void get_size(ChainServiceClient& capi, const std::string& key, persistent::version_t ver, uint32_t subgroup_index,uint32_t shard_index) {
    if constexpr (std::is_same<typename SubgroupType::KeyType,uint64_t>::value) {
...
template <typename SubgroupType>
void get_size_by_time(ChainServiceClient& capi, const std::string& key, uint64_t ts_us, uint32_t subgroup_index, uint32_t shard_index) {
    if constexpr (std::is_same<typename SubgroupType::KeyType,uint64_t>::value) {
...
template <typename SubgroupType>
void list_keys(ChainServiceClient& capi, persistent::version_t ver, uint32_t subgroup_index, uint32_t shard_index) {
...
template <typename SubgroupType>
void list_keys_by_time(ChainServiceClient& capi, uint64_t ts_us, uint32_t subgroup_index, uint32_t shard_index) {
...
template <typename SubgroupType>
void list_data_by_prefix(ChainServiceClient& capi, std::string prefix, persistent::version_t ver, uint32_t subgroup_index, uint32_t shard_index) {
    std::vector<typename SubgroupType::KeyType> keys;
    for (auto& obj : from_shard<SubgroupType,ChainServiceClient>(keys,capi,subgroup_index,shard_index,ver).where([&prefix](typename SubgroupType::ObjectType o){
...
template <>
void list_data_by_prefix<TriggerCascadeNoStoreWithStringKey>(ChainServiceClient& capi, std::string prefix, persistent::version_t ver, uint32_t subgroup_index, uint32_t shard_index) {
...
template <typename SubgroupType>
void list_data_between_versions(ChainServiceClient &capi, const std::string& key, uint32_t subgroup_index, uint32_t shard_index, persistent::version_t ver_begin, persistent::version_t ver_end) {
    if constexpr (std::is_same<typename SubgroupType::KeyType, uint64_t>::value) {
        ...
        for (auto &obj : from_versions<SubgroupType, ChainServiceClient>(static_cast<uint64_t>(std::stol(key)), capi, subgroup_index, shard_index, ver_end).where([ver_begin](typename SubgroupType::ObjectType obj) {
        ...
        for (auto &obj : from_versions<SubgroupType, ChainServiceClient>(key, capi, subgroup_index, shard_index, ver_end).where([ver_begin](typename SubgroupType::ObjectType obj) {
                    return ver_begin == INVALID_VERSION || obj.version >= ver_begin;
                }).toStdVector()) {
            std::cout << "Found:" << obj << std::endl;
        }
...
template <>
void list_data_between_versions<TriggerCascadeNoStoreWithStringKey>(ChainServiceClient &capi, const std::string& key, uint32_t subgroup_index, uint32_t shard_index, persistent::version_t ver_begin, persistent::version_t ver_end) {
    print_red("TCSS does not support list_data_between_versions.");
}

template <typename SubgroupType>
void list_data_between_timestamps(ChainServiceClient &capi, const std::string& key, uint32_t subgroup_index, uint32_t shard_index, uint64_t ts_begin, uint64_t ts_end) {
    std::vector<typename SubgroupType::KeyType> keys;
    if constexpr (std::is_same<typename SubgroupType::KeyType, uint64_t>::value) {
        ...
        for (auto &obj : from_shard_by_time<SubgroupType, ChainServiceClient>(keys, capi, subgroup_index, shard_index, ts_end).where([&key,ts_begin](typename SubgroupType::ObjectType obj) {
...
template <>
void list_data_between_timestamps<TriggerCascadeNoStoreWithStringKey>(ChainServiceClient &capi, const std::string& key, uint32_t subgroup_index, uint32_t shard_index, uint64_t ts_begin, uint64_t ts_end) {
...
template <typename SubgroupType>
void list_data_in_subgroup(ChainServiceClient& capi, uint32_t subgroup_index, persistent::version_t version) {
    std::vector<typename SubgroupType::KeyType> keys;
    std::vector<CascadeShardLinq<SubgroupType, ChainServiceClient>> shard_linq_list;

    std::unordered_map<uint32_t, std::vector<typename SubgroupType::KeyType>> shardidx_to_keys;

    for (auto &obj : from_subgroup<SubgroupType, ChainServiceClient>(shardidx_to_keys, shard_linq_list, capi, subgroup_index, version).toStdVector()) {
        std::cout << "Found:" << obj << std::endl;
    }
}

template <>
void list_data_in_subgroup<TriggerCascadeNoStoreWithStringKey>(ChainServiceClient& capi, uint32_t subgroup_index, persistent::version_t version) {
    print_red("TCSS does not support list_data_in_subgroup.");
}

template <typename SubgroupType>
void list_data_in_objectpool(ChainServiceClient& capi, persistent::version_t version, const std::string& objpool_path) {
    std::vector<typename SubgroupType::KeyType> keys;
    for (auto &obj : from_objectpool<SubgroupType, ChainServiceClient>(capi,keys,version,objpool_path).toStdVector()) {
        std::cout << "Found:" << obj << std::endl;
    }
}
#endif// HAS_BOOLINQ
...
template <typename SubgroupType>
bool perftest_ordered_put(ChainServiceClient &capi,
...
template <>
bool perftest_ordered_put<TriggerCascadeNoStoreWithStringKey>(ChainServiceClient &capi,
                          uint32_t message_size,
                          uint64_t duration_sec,
                          uint32_t subgroup_index,
                          uint32_t shard_index){
...
template <typename SubgroupType>
bool dump_timestamp(ChainServiceClient &capi,
                    uint32_t subgroup_index,
                    uint32_t shard_index,
...
using command_handler_t = std::function<bool(ChainServiceClient& capi,const std::vector<std::string>& cmd_tokens)>;
...
#define SUBGROUP_TYPE_LIST "PCSS|SCSS"
...
std::vector<command_entry_t> commands =
{
    ...
    },
    {
        ...
        "Print help info",
        "help [command name]",
        [](ChainServiceClient&,const std::vector<std::string>& cmd_tokens){
...
inline void do_command(ChainServiceClient& capi, const std::vector<std::string>& cmd_tokens) {
    try {
...
void interactive_test(ChainServiceClient& capi) {
    // loop
...
void detached_test(ChainServiceClient& capi, int argc, char** argv) {

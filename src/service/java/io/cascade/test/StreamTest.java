...
public class StreamTest {
    ...
    public static void processByteBuffers(ByteBuffer bb){
        byte b[] = new byte[bb.capacity()];
        for (int i = 0;i < bb.capacity(); i++){
            b[i] = bb.get(i);
        }
        System.out.println("values: " + new String(b));
    }

    public static void main1(){
        Client client = new Client();
        ShardSupplier supplier = new ShardSupplier(client, ServiceType.VCSS, 0, 0, -1);
        ...
        for (ByteBuffer bb: supplier.keyList()){
            byte b[] = new byte[bb.capacity()];
            for (int i = 0;i < bb.capacity(); i++){
                b[i] = bb.get(i);
                System.out.println("character " + i + " byte: " + b[i]);
            }
        ...
        Stream.generate(supplier)
              .limit(supplier.size())
              .forEach(StreamTest::processByteBuffers);
